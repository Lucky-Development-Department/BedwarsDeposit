...
public final class DepositPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        status("Plugin Modified by Molfordan (nvrlf)");
        status("Loading configs");

        ArenasConfig.setup();
        status("ArenasConfig initialized (reset per restart).");
        ...
        // ✅ Initialize log level BEFORE event registration
        String levelName = configuration.getString("log-level", "INFO").toUpperCase();
        ...
        //getServer().getPluginManager().registerEvents(new DepositWorldLoadListener(), this);
    ...
    private void createConfigWithComments(File configFile) {
        try {
            ...
            if (configFile.createNewFile()) {
                ...
                configContent.append("# DISPLAY ALL MAPS DURING PROCESSING:\n");
                configContent.append("# If enabled (true), all maps will be displayed during processing.\n");
                configContent.append("display-maps-processing: false\n");
    ...
    public void onDisable() {
        try {
            if (ArenasConfig.get() != null) {
                // Clear the YAML content
                ArenasConfig.get().getKeys(false).forEach(key -> ArenasConfig.get().set(key, null));
                ArenasConfig.save();
                status("chestLocations.yml has been cleared on disable.");
            } else {
                status("ArenasConfig not initialized, skipping clear.");
            }
        } catch (Exception e) {
            e.printStackTrace();
            status("Failed to clear chestLocations.yml: " + e.getMessage());
        }
    }
